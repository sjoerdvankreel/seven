...
private:
  void setup_stages(base::param_value const* const* automation, cv_state& cv, float bpm, 
    float& delay, float& attack, float& hold, float& decay, float& release) const;

public:
  envelope_graph(base::topology_info const* topology, std::int32_t part_index) :
  svn::base::graph_processor<base::cv_sample>(topology, part_index) {}

  bool needs_repaint(std::int32_t runtime_param) const override;
  std::int32_t sample_count(base::param_value const* state, float sample_rate, float bpm) const override;
  void process_dsp_core(base::block_input const& input, base::cv_sample* output, float sample_rate, float bpm) override;
...
public:
  lfo_graph(base::topology_info const* topology, std::int32_t part_index) :
  svn::base::graph_processor<base::cv_sample>(topology, part_index) {}
...
public:
  amp_graph(base::topology_info const* topology) :
...
public:
  cv_route_graph(base::topology_info const* topology, std::int32_t part_index) :
  svn::base::graph_processor<float>(topology, part_index) {}

  bool needs_repaint(std::int32_t runtime_param) const override;
  std::int32_t sample_count(base::param_value const* state, float sample_rate, float bpm) const override;
  void process_dsp_core(base::block_input const& input, float* output, float sample_rate, float bpm) override;
...
{
public:
  oscillator_wave_graph(base::topology_info const* topology, std::int32_t part_index) :
  svn::base::graph_processor<base::audio_sample32>(topology, part_index) {}

  bool needs_repaint(std::int32_t runtime_param) const override;
  base::param_value transform_param(std::int32_t rt_index, base::param_value value) const override;
  std::int32_t sample_count(base::param_value const* state, float sample_rate, float bpm) const override;
  void process_dsp_core(base::block_input const& input, base::audio_sample32* output, float sample_rate, float bpm) override;
...
public:
  oscillator_spectrum_graph(base::topology_info const* topology, std::int32_t part_index) :
  svn::base::graph_processor<base::audio_sample32>(topology, part_index), 
...
public:
  filter_ir_graph(base::topology_info const* topology, std::int32_t part_index) :
...
public svn::base::graph_processor<base::audio_sample32>
{
  ...
  std::vector<float> _mono;
public:
  filter_fr_graph(base::topology_info const* topology, std::int32_t part_index) :

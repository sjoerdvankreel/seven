  ...
  for(std::int32_t s = 0; s < input.sample_count; s++)
  {
    ...
    for(std::int32_t r = 0; r < audio_route_count; r++)
    {
      ...
      for (std::int32_t i = 0; i < audio_route_route_count; i++)
      {
        ...
        float amt = automation.get_modulated_dsp(audio_route_param_offset + i * 3 + 2, s, modulated);
        std::pair<std::int32_t, std::int32_t> input_ids(input_table_out[in]);
        scratch[s] = base::sanity_audio(scratch[s] + input_buffer(input_ids.first, input_ids.second)[s] * std::clamp(amt, 0.0f, 1.0f));
      }
    }
  }
  result = scratch.data();

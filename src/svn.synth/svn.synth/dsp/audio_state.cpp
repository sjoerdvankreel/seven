  ...
  std::int32_t route_index, svn::base::audio_sample32 const*& result, double& mod_time)
{
  ...
  for (std::int32_t r = 0; r < audio_route_count; r++)
  {
    ...
    if (_bank_automation[r].get_discrete(audio_route_param::on, 0) == 0) continue;
    for (std::int32_t i = 0; i < audio_route_route_count; i++)
    {
      std::int32_t input_id = _bank_automation[r].get_discrete(i * 3 + audio_route_param_offset, 0);
      if (input_id == input_off) continue;
      std::int32_t this_output_id = _bank_automation[r].get_discrete(i * 3 + 1 + audio_route_param_offset, 0);
  ...
  for (std::int32_t a = 0; a < _relevant_indices_count; a++)
  {
    ...
    for (std::int32_t s = 0; s < input.sample_count; s++)
    {
      ...
      scratch[s] = base::sanity_audio(scratch[s] + input_buffer(indices.input_ids.first, indices.input_ids.second)[s] * std::clamp(amt, 0.0f, 1.0f));
    }
  }

  ...
  base::audio_sample32 const* audio_in, base::audio_sample32* audio_out, double& mod_time)
{
  ...
  mod_time += cv.modulate(input, automation, cv_route_amp_mapping, cv_route_output::amp, 0, modulated);
  double start_time = performance_counter();
  for (std::int32_t s = 0; s < input.sample_count; s++)
  { 
    float level = automation.get_modulated_dsp(amplitude_param::level, s, modulated);
    float panning = automation.get_modulated_dsp(amplitude_param::pan, s, modulated);

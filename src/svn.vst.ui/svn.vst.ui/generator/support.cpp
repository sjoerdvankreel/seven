...
std::string
get_color_value(ui_color const& color, std::int32_t alpha_index)
{ return "#" + print_rgb_hex(color, true, alpha_index); }
std::string
get_color_name(ui_color const& color, std::int32_t alpha_index)
...
get_control_tag(topology_info const& topology, std::size_t p)
{
  ...
  std::string result = topology.parts[param.part_index].descriptor->static_name.detail;
  result += "_" + std::to_string(topology.parts[param.part_index].type_index);
  result += "_" + std::string(param.descriptor->static_name.detail);
...
std::string
print_rgb_hex(ui_color const& color, bool print_alpha, std::int32_t alpha_index)
{

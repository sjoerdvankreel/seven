...
struct discrete_descriptor
{
  ...
  std::vector<std::string> const* const items; // Items for a list parameter.
};
...
struct param_descriptor
{
  ...
  bool parse(char const* buffer, param_value& val) const;
  std::size_t format(param_value val, char* buffer, std::size_t size) const;

  // Real.
  param_descriptor(std::string const& guid, item_name const& static_name, 
    char const* unit, real_descriptor const& real, param_ui_descriptor const& ui) :
  ...
  unit(""), ui(ui), discrete(discrete_descriptor({0, 1, default_,  nullptr})) {}

  // Knob/text.
  param_descriptor(std::string const& guid, item_name const& static_name, char const* unit,
  ...
  param_descriptor(std::string const& guid, item_name const& static_name, char const* unit,
  bool knob, std::vector<std::string> const* items, std::int32_t default_, param_ui_descriptor const& ui) :
